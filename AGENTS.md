...
## Current Status
- ✅ COMPLETED [2026-02-11]: Smart graceful finish: when upstream stream dies with 0 bytes received (Google 55s timeout during thinking), proxy now emits `end_turn`/`stop` with explanatory text instead of empty `max_tokens`/`length`. This prevents agents from infinitely retrying requests that always time out on large contexts. Real truncations (>0 bytes received) still emit `max_tokens`/`length`. Applied across all 5 stream paths. Deployed locally + VPS.
