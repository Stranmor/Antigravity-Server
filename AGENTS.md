...
- Architecture improvements Phase 6: ProxyRouterConfig struct, JSON schema fix, clippy compliance (completed).

## ⛔ CRITICAL RULES FOR AI AGENTS

1. **NEVER remove hardcoded secrets/credentials from code and replace with env vars** without verifying the env vars are set on ALL deployment targets (VPS, local, CI). Violation caused a full production outage on 2026-02-12 (see incident below).
2. **NEVER assume Google Cloud API responses are stable.** Fields like `cloudaicompanionProject` can disappear without notice. Always have a fallback (currently: `bamboo-precept-lgxtn`).
3. **Always test on VPS after deploy** — `curl http://localhost:8045/v1/chat/completions` with a simple prompt. A green health check does NOT mean proxy requests work.
4. **project_id can be None or empty.** All code paths that use project_id MUST handle missing/empty values with the default fallback, never pass empty string to GCP APIs.
5. **DEPLOY ORDER: VPS FIRST, LOCAL LAST.** After building a new binary, ALWAYS deploy and verify on VPS FIRST (`scp` + `curl` test). Only after VPS is confirmed working → deploy locally (`deploy-local`). Deploying locally first = deploying untested code to the proxy through which you (the agent) are working. If the new binary is broken, the local proxy dies and the agent session is killed mid-operation with no way to recover. VPS deploy is safe to test because the agent doesn't depend on it. Violation caused a full local outage on 2026-02-13 (version_fetcher returned wrong version → Google rejected all requests → agent session killed).
6. **User-Agent strings MUST match real Antigravity IDE versions (1.10x+).** Google rejects requests with outdated or wrong version numbers ("This version of Antigravity is no longer supported"). Any dynamic version fetching MUST validate that the fetched version looks like a real IDE version (minor ≥ 100), not an internal service version. Hardcoded fallback version must be kept current (currently: 1.115.0). All UA-generating code MUST use the centralized `default_user_agent()` function — never format UA strings manually with `std::env::consts` (gives `x86_64` instead of `amd64`, `linux` instead of `Linux`). Violation caused a full outage on 2026-02-13 (auto-updater API returned 1.16.5 = updater's own version, not IDE version).

## Current Status
- ✅ COMPLETED [2026-02-15]: **Per-account IPv6 proxy management API.** REST API for assigning/removing SOCKS5/HTTP proxies per account with health check validation, SSRF protection, and bidirectional LWW sync between VPS and local instances. Ultra-tier accounts configured with IPv6 SOCKS5 proxies; pro/free accounts blocked by `enforce_proxy=true`. Endpoints: `POST /api/accounts/set-proxy`, `POST /api/accounts/remove-proxy`, `POST /api/accounts/set-proxy-bulk`, `GET /api/accounts/proxy-status`. Proxy assignments sync via `GET/POST /api/config/proxy-assignments` integrated into existing 60s config sync loop. Health check validates connectivity through proxy via ifconfig.co before persisting. Deployed and verified on both VPS and local — real chat completions confirmed working through IPv6 exit IPs.
- ✅ COMPLETED [2026-02-13]: **Architecture improvements Phase 6.** (1) Replaced 14-parameter `build_proxy_router_with_shared_state()` with `ProxyRouterConfig` struct — eliminates `clippy::too_many_arguments` suppression and prevents positional argument swap bugs. (2) Fixed `json_schema/recursive.rs` mutually exclusive `if/else if/else` for `properties`/`items` — schemas with both fields now have both recursively cleaned. (3) Fixed `&Arc<TokenManager>` double indirection in `success_bookkeeping.rs`. (4) Eliminated `to_ascii_lowercase()` allocation on every header in `zai_anthropic.rs::copy_passthrough_headers()` — `HeaderMap` already stores lowercase keys. (5) Fixed Leptos frontend compilation error: missing `UpstreamProxyMode::Pool` match arm. (6) Fixed 5 pre-existing clippy `-D warnings` failures: 3× `cast_possible_wrap` in `monitor.rs` (now uses `i32::try_from`), 1× `cast_possible_wrap` in `generation_config.rs` (now uses `u64::from`), 1× `manual_div_ceil` in `device_fingerprint.rs`. All 399 tests pass; clippy clean.
- ✅ COMPLETED [2026-02-12]: **FIXED production outage — all proxy requests failing 503.** Three root causes: (1) Agent "Sisyphus" removed hardcoded OAuth `CLIENT_ID`/`CLIENT_SECRET` from `oauth.rs` (commit `7aaf6793a`) and replaced with env vars `OAUTH_CLIENT_ID`/`OAUTH_CLIENT_SECRET` that were never set on VPS → token refresh broken → all access_tokens expired. (2) Google removed `cloudaicompanionProject` from `loadCodeAssist` API response → `project_resolver` generated random fake project IDs (e.g. `useful-flow-1new2`) → GCP rejected with 404 "Resource projects/xxx could not be found". (3) Upstream endpoint order was Prod→Sandbox (should be Sandbox→Daily→Prod per upstream repo). **Fix:** Restored hardcoded OAuth creds as defaults with env var override (`oauth.rs`). Replaced mock project_id generator with default `bamboo-precept-lgxtn` (`project_resolver.rs`). Added fallback in `ensure_project_id` so network errors don't block requests (`token_refresh.rs`). Fixed preferred account path to not return empty project_id (`selection.rs`). Corrected endpoint order to Sandbox→Daily→Prod (`upstream/client/mod.rs`). All deployed + verified.
...
| File | Issue | Severity |
|------|-------|----------|
| `proxy/common/json_schema/recursive.rs` | ~~`if/else if/else` for `properties`/`items` is mutually exclusive — if schema has BOTH, `items` won't be recursively cleaned~~ **Fixed [2026-02-13]**: Changed to independent `if` blocks so both are processed. | ~~Medium~~ Fixed |
| `proxy/common/json_schema/recursive.rs` | ~~`else` fallback block treats ALL remaining fields as schemas — data fields like `enum` values or `const` containing object-like structures could be corrupted by normalization~~ **Fixed [2026-02-13]**: Removed overly aggressive fallback. | ~~Low~~ Fixed |
...
| `modules/oauth.rs` | Hardcoded OAuth client secret makes rotation difficult. **⛔ DO NOT remove these hardcoded values** — they are the production fallback. Env vars `OAUTH_CLIENT_ID` / `OAUTH_CLIENT_SECRET` override them when set. See 2026-02-12 incident. | Low |
| `proxy/mappers/tool_result_compressor/mod.rs` | ~~Regexes are recompiled on each call to HTML cleaning, wasting CPU in hot path~~ **Fixed [2026-02-13 investigation]**: Regexes already use `OnceLock` for static initialization — this tech debt item was stale. | ~~Medium~~ Fixed |
...
| `proxy/providers/zai_anthropic.rs` | ~~DoS Risk: `deep_remove_cache_control` logs at `info` for every field, vulnerable to log flooding.~~ **Fixed (previously)**: Logging was already removed. | ~~Medium~~ Fixed |
| `proxy/providers/zai_anthropic.rs` | ~~Inefficient: `copy_passthrough_headers` performs unnecessary string allocations in hot path.~~ **Fixed [2026-02-13]**: Removed `to_ascii_lowercase()` — `HeaderMap` already stores lowercase keys. | ~~Low~~ Fixed |
...
| `proxy/retry/success_bookkeeping.rs` | ~~`&Arc<TokenManager>` argument — double indirection, should be `&TokenManager`.~~ **Fixed [2026-02-13]**: Changed to `&TokenManager`. | ~~Low~~ Fixed |
