...
## Current Status
- ✅ COMPLETED [2026-02-11]: Smart graceful finish v2: ANY stream error now emits `end_turn`/`stop` with timeout explanation text, unconditionally. V1 used byte-counting (0 bytes → end_turn, >0 bytes → max_tokens), but production showed thinking-only events (2563 bytes of thinking_delta with zero text output) were misclassified as "real truncations", causing agents to still retry infinitely. V2 removes all byte-counting logic — simpler and catches all timeout scenarios. Applied across all 5 stream paths. Deployed locally + VPS.
