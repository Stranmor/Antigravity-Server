    ...
    if response.status().is_success() {
        ...
        if token_res.refresh_token.is_none() {
            crate::modules::logger::log_warn(
                r"Warning: Google did not return refresh_token. Possible reasons:
...
pub async fn refresh_access_token(refresh_token: &str) -> Result<TokenResponse, String> {
    ...
    if response.status().is_success() {
        let token_data = response
            .json::<TokenResponse>()
            .await
            .map_err(|e| format!("Refresh data parse failed: {}", e))?;

        crate::modules::logger::log_info(&format!(
            "Token refresh successful! Valid for: {} seconds",
...
pub async fn ensure_fresh_token(
    ...
    Ok(crate::models::TokenData::new(
        ...
        current_token.refresh_token.clone(), // Refresh may not return new refresh_token
        response.expires_in,
