...
pub fn transform_claude_request_in(
    ...
    // [FIX] Strip Gemini-only dummy signatures for Claude models on Vertex AI.
    // `skip_thought_signature_validator` is a Gemini-specific bypass value.
    // Claude models on Vertex reject it with "Invalid `signature` in `thinking` block".
    if mapped_model.starts_with("claude-") {
        strip_non_claude_thought_signatures(&mut body);
    }
...
fn strip_non_claude_thought_signatures(value: &mut Value) {
