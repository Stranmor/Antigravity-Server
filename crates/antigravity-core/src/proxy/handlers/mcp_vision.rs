...
async fn handle_vision_post(state: AppState, headers: HeaderMap, body: Body) -> Response {
    ...
    if is_initialize_request(&request_json) {
        return handle_initialize(&state, &request_json, id).await;
    }
    ...
    handle_method(&state, method, &request_json, id).await
}

async fn handle_initialize(state: &AppState, request_json: &Value, id: Value) -> Response {
...
async fn handle_tools_call(state: &AppState, request_json: &Value, id: Value) -> Response {
    ...
    match crate::proxy::zai_vision_tools::call_tool(&zai, upstream_proxy, timeout, tool_name, &arguments).await {
        Ok(tool_result) => (StatusCode::OK, axum::Json(jsonrpc_result(id, tool_result))).into_response(),
        Err(e) => (
            StatusCode::OK,
